...
public class RdsFileUtil {
    /**
     * 把一个目录打包到一个指定的zip文件中
     * @param zipFilename    打包后zip文件绝对路径
     * @param dirpath        待打包文件绝对路径
     * @param pathName       打包后zip文件内部压缩路径
     */
    public static void packToolFiles(String zipFilename,String dirpath,String pathName) throws FileNotFoundException, IOException {
        File f = new File(zipFilename);
        ByteArrayOutputStream tempbaos = null;
        BufferedOutputStream tempbos = null;
        ZipArchiveOutputStream zaos=null;
        try{
        zaos = new ZipArchiveOutputStream(new BufferedOutputStream(new FileOutputStream(f)));
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (files[i].isDirectory()) {
                packToolFiles(zipFilename,files[i].getAbsolutePath(), pathName + files[i].getName() + File.separator);
            } else {
    ...
    /**
     * 解压RAR文件
     * @param destDir 解压至目标文件夹
     * @param sourceRar 解压源文件
     */
    ...
    public static Map<String,Map<String,String>> getFilesByExtension(String directory,String extension) throws IOException {
        String[] strs = new String[]{extension};
        Iterator<File> it = FileUtils.iterateFiles(new File(directory), strs, false);
    ...
    public static List<File> PDFToJpg(File file,String ext_flag) throws Exception {
        //输出文件
        ...
        for(int i=0;i<pages.size();i++){
            ...
            if("Y".equals(ext_flag)){
                outFile = new File(file.getAbsolutePath().replaceAll(".pdf","ext.jpg"));
            }else{
                outFile = new File(file.getAbsolutePath().replaceAll(".pdf",".jpg"));
            }
    ...
    public static String getMd5ByFile(String path) throws IOException {
        ...
        String md5 = DigestUtils.md5Hex(org.apache.commons.io.IOUtils.toByteArray(fis));
